...
import com.dtssAnWeihai.adapter.GonglueAdapter.ViewHolder;
...
public class ImageLoader {
	...
	public void loadImage(String url, BaseAdapter adapter, ViewHolder holder) {
		...
		if (bitmap == null) {
			holder.image.setImageResource(R.drawable.ic_launcher);//缓存没有设为默认图片
			ImageLoadTask imageLoadTask = new ImageLoadTask();
			imageLoadTask.execute(url, adapter, holder);
		} else {
			Bitmap zoomImage = MyTools.zoomImage(bitmap, 120, 100);
			holder.image.setImageBitmap(zoomImage);//设为缓存图片
		}
...
//			Bitmap drawable = MyTools.returnBitMap(url);
			Bitmap drawable = loadImageFromInternet(url);// 获取网络图片
			Bitmap zoomImage = MyTools.zoomImage(drawable, 120, 100);
			return zoomImage;
		}
